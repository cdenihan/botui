  ...
  Future<void> enable() async {
    ...
    try {
      ...
      if (!state.isActive) {
        state = state.copyWith(
          isLoading: false,
          errorMessage: () => 'LAN only mode did not become active. Check ethernet cable and DHCP service status.',
        );
        log.warning('LAN only mode setup completed but mode is not active');
        return;
      }
